...
module.exports = class President {
    async getImage(image) {
        ...
        let bg = await jimp.read(`${__dirname}/../../assets/president.png`);
        let img = await jimp.read(image);
        const headX = bg.bitmap.width * 0.35;
  const headY = bg.bitmap.height * 0.06;
  const headW = bg.bitmap.width * 0.30;
